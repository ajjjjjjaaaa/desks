...
ShellRoot {
    ...
    Variants {
        ...
        PanelWindow {
            ...
            readonly property list<var> windowRegions: filterWindowRegionsByLayers(root.windows.filter(w => w.workspace.id === panelWindow.activeWorkspaceId), panelWindow.layerRegions).map(window => {
                return {
            ...
            readonly property list<var> layerRegions: {
                ...
                const nonBarTopLayers = topLayers.filter(layer => !(layer.namespace.includes(":bar") || layer.namespace.includes(":dock"))).map(layer => {
                    return {
                        at: [layer.x, layer.y],
            ...
            Process {
                ...
                command: ["bash", "-c", `${Directories.scriptPath}/images/find_regions.py ` + `--hyprctl ` + `--image '${StringUtils.shellSingleQuoteEscape(panelWindow.screenshotPath)}' ` + `--max-width ${Math.round(panelWindow.screen.width * root.falsePositivePreventionRatio)} ` + `--max-height ${Math.round(panelWindow.screen.height * root.falsePositivePreventionRatio)} `]
                ...
                MouseArea {
                    ...
                    Repeater {
                        ...
                        delegate: TargetRegion {
                            ...
                            required property var modelData
                            showIcon: true
                            targeted: !panelWindow.draggedAway && (panelWindow.targetedRegionX === modelData.at[0] && panelWindow.targetedRegionY === modelData.at[1] && panelWindow.targetedRegionWidth === modelData.size[0] && panelWindow.targetedRegionHeight === modelData.size[1])
