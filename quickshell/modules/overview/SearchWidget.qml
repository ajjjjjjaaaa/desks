...
Item { // Wrapper
    ...
    Process {
        id : mathProcess
        property list < string > baseCommand : [
            "qalc",
            "-t"
        ]
    ...
    Keys.onPressed : event => { // Prevent Esc and Backspace from registering
        ...
        if (event.key === Qt.Key_Backspace) {
            if (!searchInput.activeFocus) {
                ...
                if (event.modifiers & Qt.ControlModifier) { // Delete word before cursor
                    let text = searchInput.text
                    let pos = searchInput.cursorPosition
                    if (pos > 0) { // Find the start of the previous word
                ...
                } else { // Delete character before cursor if any
                    if (searchInput.cursorPosition > 0) {
                        searchInput.text = searchInput.text.slice(0, searchInput.cursorPosition - 1) + searchInput
                            .text
                            .slice(searchInput.cursorPosition)
                            searchInput
                            .cursorPosition -= 1
                    }
                } // Always move cursor to end after programmatic edit
                searchInput.cursorPosition = searchInput
                    .text
                    .length
                    event
                    .accepted = true
        ...
        if (event.text && event.text.length === 1 && event.key !== Qt.Key_Enter && event.key !== Qt.Key_Return && event.text.charCodeAt(0) >= 0x20) { // ignore control chars like Backspace, Tab, etc.
            if (!searchInput.activeFocus) {
                searchInput.forceActiveFocus() // Insert the character at the cursor position
                searchInput.text = searchInput.text.slice(0, searchInput.cursorPosition) + event.text + searchInput
                    .text
                    .slice(searchInput.cursorPosition)
                    searchInput
                    .cursorPosition += 1
    ...
    Rectangle { // Background
        ...
        topLeftRadius : root.showResults
            ? Appearance.rounding.verysmall
            : Appearance.rounding.small
        topRightRadius : root.showResults
            ? Appearance.rounding.verysmall
            : Appearance.rounding.small
        ColumnLayout {
            id : columnLayout
            ...
            StyledListView { // App results
                ...
                model : ScriptModel {
                    ...
                    values : { // Search results are handled here // //////////////// Skip? //////////////////
                        if (root.searchingText == "") 
                            return []
                         // /////////// Special cases ///////////////
                        if (root.searchingText.startsWith(Config
                            .options
                            .search
                            .prefix
                            .clipboard)) { // Clipboard
                            const searchString = root.searchingText.slice(Config
                                .options
                                .search
                                .prefix
                                .clipboard
                            ...
                            execute: () => {
                                const cleanedCommand = root
                                    ...
                                    .execDetached([
                                        "bash",
                                        "-c",
                                        searchingText.startsWith('sudo')
                                            ? `${
                                                Config
                                                    .options
                                                    .apps
                                                    .terminal
                                            } fish -C '${cleanedCommand}'`
                                            : cleanedCommand
                                        ])
                            }
                        }
                        const launcherActionObjects = root
                            .searchActions
                            .map(action => {
                                const actionString = `${
                                    Config
                                ...
                                if (actionString.startsWith(root.searchingText) || root.searchingText.startsWith(actionString)) {
                                    return {
                                        name: root.searchingText.startsWith(actionString)
                                            ? root.searchingText
                                            : actionString,
                                        clickActionName: "Run",
                                        type: "Action",
                        ...
                        result = result.concat(launcherActionObjects) // ///////// Math result & command //////////
                        ...
                        result.push({
                            ...
                            execute: () => {
                                let url = Config
                                    .options
                                    .search
                                    .engineBaseUrl + root.searchingText
                ...
                delegate : SearchItem { // The selectable item for each search result
                    ...
                    entry : modelData
                    query : root.searchingText.startsWith(Config
                        .options
                        .search
                        .prefix
                        .clipboard)
                            ? root.searchingText.slice(Config
                                .options
                                .search
                                .prefix
                                .clipboard
            ...
            RowLayout {
                ...
                MaterialSymbol {
                    ...
                    text : root.searchingText.startsWith(Config
                        .options
                        .search
                        .prefix
                        .clipboard)
                ...
                TextField { // Search box
                    ...
                    Behavior on implicitWidth {
                        ...
                        onAccepted : {
                            if (appResults.count > 0) { // Get the first visible delegate and trigger its click
                        ...
                        Component.onCompleted : { // Force focus when component is ready
