...
Singleton {
    ...
    function addApiKeyAdvice(model) {
        root.addMessage(StringUtils.format(qsTr('To set an API key, pass it with the command\n\nTo view the key, pass "get" with the command<br/>\n\n### For {0}:\n\n**Link**: {1}\n\n{2}'), model.name, model.key_get_link, model.key_get_description ?? qsTr("<i>No further instruction provided</i>")), Ai.interfaceRole);
    }
    ...
    function setModel(modelId, feedback = true) {
        ...
        if (modelList.indexOf(modelId) !== -1) {
            ...
            if (feedback)
                root.addMessage(StringUtils.format(StringUtils.format("Model set to {0}"), model.name), root.interfaceRole);
    ...
    Process {
        ...
        function makeRequest() {
            ...
            const requestCommandString = `curl --no-buffer "${endpoint}"` + ` ${headerString}` + ((apiFormat == "gemini") ? "" : ` -H "Authorization: Bearer \$\{${root.apiKeyEnvVarName}\}"`) + ` -d '${StringUtils.shellSingleQuoteEscape(JSON.stringify(data))}'`;
        ...
        function parseGeminiBuffer() {
            ...
            try {
                if (requester.geminiBuffer.length === 0)
                    return;
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            try {
                // to parse full response into json for error handling
    ...
    function handleGeminiFunctionCall(name, args) {
        if (name === "switch_to_search_mode") {
            ...
            root.addMessage(qsTr("Unknown function call: {0}"), "assistant");
    }
}
